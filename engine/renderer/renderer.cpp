...
void Renderer::init(GLFWwindow *window, tGrid* worldMatrix) {
  this->window = window;
...
Renderer::~Renderer() {
  // cleanup of all resources used in render loop
  device.waitIdle();
}
// ???????????????????????????????????
// void (*Renderer::updateFunc) (std::array<Cell, GRID_SIZE_X * GRID_SIZE_Y>&) =
// nullptr;
...
void Renderer::updateCells() {
  vk::DeviceSize bufferSize = sizeof(tGrid);

  memcpy(storageBufferWriteLoc, m_WorldMatrix, bufferSize);
}

void Renderer::drawFrame() {
  // the * operator on vk::raii::<something> returns vk::<something> & (remember
  // it's a reference)
  while (device.waitForFences(*inFlightFences[currentFrame], VK_TRUE,
                              UINT32_MAX) == vk::Result::eTimeout)
    ;
  ...
  if (result == vk::Result::eErrorOutOfDateKHR) {
    ...
    throw std::runtime_error(
        "[Swapchain]: Failed to acquire a swapchain image.");
  }
  ...
  vk::SubmitInfo submitInfo(waitSemaphores, waitStages,
                            *commandBuffers[currentFrame], signalSemaphores);
...
void Renderer::recreateSwapchain() {
  ...
  // The reason we're doing device wait idle instead of just waiting on a fence
  // is we don't know what state the gpu is in
  device.waitIdle();
...
void Renderer::createInstance() {
  ...
  vk::InstanceCreateInfo createInfo{
      ...
      static_cast<uint32_t>(enableValidationLayers ? ValidationLayers.size()
                                                   : 0),
      // enabledLayerCount;
  ...
  if (enableValidationLayers && !checkValidationLayerSupport()) {
    throw std::runtime_error(
        "[VK_Instance]: Validation Layers requested but not available!");
  }
}
...
void Renderer::pickPhysicalDevice() {
  ...
  if (devices.size() == 0) {
    throw std::runtime_error(
        "[VK_Instance]: No Graphics Devices with Vulkan support!");
  }
  ...
  if (candidates.rbegin()->first > 0) {
    ...
    throw std::runtime_error("[VK_Instance]: No Supported Graphics Devices "
                             "with required Queue Families!");
  } else if (candidates.rbegin()->first == -2) {
    throw std::runtime_error("[VK_Instance]: No Supported Graphics Devices "
                             "with required Device Extensions!");
  } else if (candidates.rbegin()->first == -3) {
    throw std::runtime_error("[VK_Instance]: No Supported Graphics Devices "
                             "with required Swap Chain Support!");
  } else {
    throw std::runtime_error(
        "[VK_Instance]: No Supported Graphics Devices found!");
  }
}
...
void Renderer::createSwapchain() {
  ...
  vk::PresentModeKHR presentMode =
      choostSwapPresentMode(swapchainSupport.presentModes);
  ...
  QueueFamilyIndices indices = queryQueueFamilyIndices(physicalDevice);
  uint32_t queueFamilyIndices[2] = {indices.graphicsFamily.value(),
                                    indices.presentFamily.value()};
...
void Renderer::createDescriptorSetLayout() {
  ...
  std::vector<vk::DescriptorSetLayoutBinding> layoutBindings = {
      uboLayoutBinding, ssboLayoutBinding};
...
void Renderer::createGraphicsPipeline() {
  std::vector<char> vertexShaderCode =
      readFile(utils::getExecutableDir() / "res/shaders/testv.spv");
  std::vector<char> fragmentShaderCode =
      readFile(utils::getExecutableDir() / "res/shaders/testf.spv");

  vk::raii::ShaderModule vertShaderModule =
      createShaderModule(vertexShaderCode);
...
[[nodiscard]] auto
Renderer::createBuffer(vk::DeviceSize _size, vk::BufferUsageFlags _usage,
...
void Renderer::copyBuffer(vk::raii::Buffer &_src, vk::raii::Buffer &_dst,
                          vk::DeviceSize _size) {
  ...
  copyCommandBuffer.copyBuffer(
      *_src, *_dst, std::array<vk::BufferCopy, 1>{vk::BufferCopy(0, 0, _size)});
  copyCommandBuffer.end();

  graphicsQueue.submit(std::array<vk::SubmitInfo, 1>{
      vk::SubmitInfo(nullptr, nullptr, *copyCommandBuffer, nullptr)});
...
void Renderer::createUniformBuffers() {
  ...
  std::tie(uniformBuffer, uniformBuffersMemory) =
...
void Renderer::createStorageBuffer() {
  ...
  std::tie(storageBuffer, storageBufferMemory) =
...
void Renderer::createDescriptorSet() {
  ...
  vk::DescriptorBufferInfo uniformBufferInfo(*uniformBuffer, 0,
                                             sizeof(UniformBufferObject));
  vk::DescriptorBufferInfo storageBufferInfo(*storageBuffer, 0,
                                             sizeof(tGrid));

  vk::WriteDescriptorSet descriptorWrite(
      *descriptorSet, 0, 0, vk::DescriptorType::eUniformBuffer, nullptr,
      uniformBufferInfo, nullptr, nullptr);
  device.updateDescriptorSets(descriptorWrite, nullptr);

  descriptorWrite = vk::WriteDescriptorSet(
      *descriptorSet, 1, 0, vk::DescriptorType::eStorageBuffer, nullptr,
      storageBufferInfo, nullptr, nullptr);
  device.updateDescriptorSets(descriptorWrite, nullptr);
}
...
void Renderer::createSyncObjects() {
  ...
  vk::FenceCreateInfo fenceCreateInfo{
      vk::FenceCreateFlagBits::eSignaled // flags	-> first frame waits for
                                         // a signaled fence
  };
...
void Renderer::recordCommandBuffer(vk::raii::CommandBuffer &_commandBuffer,
  ...
  vk::RenderPassBeginInfo renderPassBeginInfo{
      ...
      *swapchainFramebuffers[_imageIndex], // framebuffer
      vk::Rect2D{                          // renderArea
                 vk::Offset2D{0, 0}, swapchainImageExtent},
      1,          // clearValueCount
  ...
  _commandBuffer.drawIndexed(indices.size(), ubo.grid_size.x * ubo.grid_size.y,
                             0, 0, 0);
...
int Renderer::rateDeviceSuitability(const vk::raii::PhysicalDevice _device) {
  ...
  if (queryQueueFamilyIndices(_device).graphicsFamily ==
      queryQueueFamilyIndices(_device).presentFamily) {
    // better performance if graphics and presentation is done on the same queue
    score += 50;
    // It may actually be faster with multiple queues running concurrently but
    // that would require manual tranfer of ownership. Look into it later.
  }
...
Renderer::queryQueueFamilyIndices(vk::raii::PhysicalDevice _device) {
...
Renderer::querySwapchainSupport(vk::raii::PhysicalDevice _device) {
...
// Check if Device Extensions required by the application are supported by the
// device.
bool Renderer::checkDeviceExtensionSupport(vk::raii::PhysicalDevice _device) {
  ...
  std::vector<vk::ExtensionProperties> availableExtensions =
      _device.enumerateDeviceExtensionProperties();

  // Make a set of all required extensions and remove them from the set if
  // they're found. If all required extensions are found, the set is empty.
...
// Returns the way in which the Swapchain present images to the screen (Vsync,
// Triple Buffering etc.)
vk::PresentModeKHR
Renderer::choostSwapPresentMode(std::vector<vk::PresentModeKHR> _presentModes) {
...
vk::Extent2D
Renderer::chooseSwapExtent(const vk::SurfaceCapabilitiesKHR _capabilities) {
  if (_capabilities.currentExtent.width !=
    ...
    vk::Extent2D out = {static_cast<uint32_t>(width),
                        static_cast<uint32_t>(height)};

    out.width = std::clamp(out.width, _capabilities.maxImageExtent.width,
                           _capabilities.minImageExtent.width);
    out.height = std::clamp(out.height, _capabilities.maxImageExtent.height,
...
// Check if Instance Extensions required by the application are supported by the
// Vulkan Implementation and return them.
std::vector<const char *> Renderer::getRequiredInstanceExtensions() {
  ...
  // Lambda to check if the required instance extension is supported by the
  ...
  for (uint32_t i = 0; i < glfwExtensionCount; ++i) {
    if (!checkInstanceExtensionSupport(glfwExtensions[i])) {
      throw std::runtime_error(
          "[VK_Instance]: Extensions required by GLFW not available!");
    }
  }
  ...
  if (enableValidationLayers) {
    if (!checkInstanceExtensionSupport(VK_EXT_DEBUG_UTILS_EXTENSION_NAME)) {
      throw std::runtime_error(
          "[VK_Instance]: VK_EXT_debug_utils not available!");
    }
    ...
    if (checkInstanceExtensionSupport(
      ...
      std::cout << "[VK_Instance]: VK_KHR_get_physical_device_properties2 "
                   "couldn't be loaded. Find stack trace to this function lol.";
    }
    // need to include this extension to prevent this warning(possibly wrong):
    // [Validation Layer]: vkGetPhysicalDeviceProperties2KHR: Emulation found
    // unrecognized structure type in pProperties->pNext - this struct will be
    // ignored Genuinely think this is a bug with the vulkanSDK cuz it throws on
    // the vkCreateDevice call which then calls vkGetPhysicalDeviceProperties
  }
...
std::vector<char> Renderer::readFile(const std::filesystem::path &filename) {
  ...
  if (!file.is_open()) {
    throw std::runtime_error(std::string("[CPU]: Failed to open file at: ") +
                             filename.string());
  }
